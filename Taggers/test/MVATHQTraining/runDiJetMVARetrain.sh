...
# run the script
fggRunJobs.py --load VBFMVATrainingSamples.json -d ${WORKSPACE}/test_diphodijet_training -x cmsRun VBFDiPhoDiJetMVA_Training.py maxEvents=1000 -n 500 -q ${queue} -D -P useAAA=1 --no-use-tarball
echo 
...
fggRunJobs.py --load VBFMVATrainingSamples.json -d \
    ...
    cmsRun VBFDiPhoDiJetMVA_Compare.py maxEvents=1000 -n 500 -q ${queue} -D -P useAAA=1 --no-use-tarball
