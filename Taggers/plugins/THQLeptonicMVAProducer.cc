...
namespace flashgg {
    ...
    class THQLeptonicMVAProducer : public EDProducer
    {
        
    public:
        THQLeptonicMVAProducer( const ParameterSet & );
    private:
    ...
    THQLeptonicMVAProducer::THQLeptonicMVAProducer( const ParameterSet &iConfig ) :
        ...
        _pujid_wp_pt_bin_3  ( iConfig.getParameter<std::vector<double> > ( "pujidWpPtBin3" ) )
    {
        ...
        if (_MVAMethod != ""){
            ...
            ThqLeptonicMva_->AddVariable( "dijet_LeadJPt"     , &dijet_LeadJPt_    );
            ThqLeptonicMva_->AddVariable( "dijet_SubJPt"      , &dijet_SubJPt_     );
            ThqLeptonicMva_->AddVariable( "dijet_abs_dEta"    , &dijet_abs_dEta_   );
            //ThqLeptonicMva_->AddVariable( "dijet_dy"          , &dijet_dy_         );
            ThqLeptonicMva_->AddVariable( "dijet_Mjj"         , &dijet_Mjj_        );
            ThqLeptonicMva_->AddVariable( "dijet_Zep"         , &dijet_Zep_        );
            //ThqLeptonicMva_->AddVariable( "dijet_minDRJetPho" , &dijet_minDRJetPho_);
            //ThqLeptonicMva_->AddVariable( "dijet_dipho_dphi"  , &dijet_dipho_dphi_ );
            ThqLeptonicMva_->AddVariable( "dijet_dPhi_trunc"  , &dijet_dphi_trunc_ );
            //ThqLeptonicMva_->AddVariable( "dipho_PToM"        , &dipho_PToM_       );
            ThqLeptonicMva_->AddVariable( "leadPho_PToM"      , &leadPho_PToM_);
            ThqLeptonicMva_->AddVariable( "sublPho_PToM"      , &sublPho_PToM_);
            //new variables
            ThqLeptonicMva_->BookMVA( _MVAMethod.c_str() , thqleptonicMVAweightfile_.fullPath() );
        }
        ...
        produces<vector<THQLeptonicMVAResult> >();
        
    }
    
    void THQLeptonicMVAProducer::produce( Event &evt, const EventSetup & )
    {
        ...
        std::auto_ptr<vector<THQLeptonicMVAResult> > thqleptonic_results( new vector<THQLeptonicMVAResult> );
        for( unsigned int candIndex = 0; candIndex < diPhotons->size() ; candIndex++ ) {
            
            flashgg::THQLeptonicMVAResult mvares;
            ...
            std::vector<edm::Ptr<Electron> > goodElectrons = selectElectrons( theElectrons->ptrs(), diPhotons->ptrAt( candIndex ),  vertices->ptrs(),
                leptonPtThreshold_, 
            ...
            if( goodElectrons.size()>0 ) {
                ...
                for( unsigned int ElectronIndex = 0; ElectronIndex < goodElectrons.size(); ElectronIndex++ ) {
                    ...
                    for( unsigned int JetIndex = 0; JetIndex < Jets[jetCollectionIndex]->size() ; JetIndex++ ) {
                        ...
                        if( _usePuJetID && !thejet->passesPuJetId(diPhotons->ptrAt( candIndex ))){ continue;}
                        if( _useJetID ){
                            if( _JetIDLevel == "Loose" && !thejet->passesJetID  ( flashgg::Loose ) ) continue;
                            if( _JetIDLevel == "Tight" && !thejet->passesJetID  ( flashgg::Tight ) ) continue;
                        }
                        ...
                        particles.push_back( math::RhoEtaPhiVector( thejet->pt(), thejet->eta(), thejet->phi() ) );

                    }//end of jets loop
                    //numElectronJetsdR.push_back( deltaRElectronJetcount );

                    //tagElectrons.push_back( Electron );
                    particles.push_back( math::RhoEtaPhiVector( Electron->pt(), Electron->eta(), Electron->phi() ) );
            ...
            std::cout << "shapes : " << shapeVars.aplanarity()  << " "<< shapeVars.circularity() << " "<< shapeVars.isotropy() << std::endl;
            ...
            if (_MVAMethod != "") 
                mvares.thqLeptonicMvaResult_value = ThqLeptonicMva_->EvaluateMVA( _MVAMethod.c_str() );
            ...
            thqleptonic_results->push_back( mvares );
        }
        evt.put( thqleptonic_results );
    }
}

typedef flashgg::THQLeptonicMVAProducer FlashggTHQLeptonicMVAProducer;
DEFINE_FWK_MODULE( FlashggTHQLeptonicMVAProducer );
// Local Variables:
// mode:c++
